...
def main():
    ...
    for match in matches:
        ...
        final_path = options.output_path+'gauss-evt-'+match.group(0)+'.qmt'  
        out = open(final_path, "w")
        out.write( re.sub(change_regex, match.group(0), model) )
        out.close()
        logging.info('File created :'+options.output_path+'gauss-evt-'+match.group(0)+'.qmt')
