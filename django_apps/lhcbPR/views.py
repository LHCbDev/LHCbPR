...
def joblistInfo(request, app_name, desc_id, plat_id, host_id):
   ...
   #print job_query
    ...
    info_query = "SELECT LHCBPR_JOBRESULTS.JOB_ID, \
      ...
      INNER JOIN LHCBPR_JOB \
      ON LHCBPR_JOB.ID = LHCBPR_JOBRESULTS.JOB_ID \
      WHERE LHCBPR_JOBATTRIBUTE.\"GROUP\" = 'JobInfo' "

    info_query += """AND LHCBPR_JOB.JOBDESCRIPTION_ID = %s
      ORDER BY LHCBPR_JOBRESULTS.JOB_ID,
      LHCBPR_JOBATTRIBUTE.NAME""" % desc_id
   ...
   #print file_query
    ...
    group_query = "SELECT DISTINCT LHCBPR_JOBRESULTS.JOB_ID, \
     ...
     INNER JOIN LHCBPR_JOB \
     ON LHCBPR_JOB.ID = LHCBPR_JOBRESULTS.JOB_ID \
     WHERE LHCBPR_JOB.JOBDESCRIPTION_ID = %s \
     ORDER BY LHCBPR_JOBRESULTS.JOB_ID" % desc_id
